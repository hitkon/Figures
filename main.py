...
class App(tkinter.Tk):
    ...
    def __init__(self):
        ...
        self.bind('<KeyRelease-Alt_L>', self.set_no_regular_in_alt_release)
    ...
    def figure_chooser(self, event):
        ...
        if self.cur_fig_name == 'Polygon':
            ...
            return self.tags[max_tag]
        if self.cur_fig.name == 'Polygonal Line':
            for i in self.intervals:
                i.delete(self.canvas)
            self.tags[max_tag] = PolygonalLine(points, self.canvas_current_line_color, max_tag)
            return self.tags[max_tag]

    def figure_chooser_left_click(self,event):
        if self.cur_fig.name == 'Polygonal Line':
            pass
    ...
    def on_shift_press(self, event):
        if self.cur_fig.name == 'Polygonal Line':
            self.points.append(Point(event.x, event.y))
            self.cur_fig_name = 'Interval'
            if self.canvas_cur_fig:
                self.canvas_cur_fig.delete(self.canvas)
            self.intervals.append(self.figure_chooser(event))
            self.intervals[-1].draw(self.canvas)
            self.canvas_old_coords = event.x, event.y
            self.cur_fig_name = 'Polygonal Line'
